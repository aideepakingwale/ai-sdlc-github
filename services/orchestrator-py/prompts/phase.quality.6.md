...
- Tests accompany the code: arrange-act-assert, meaningful names stating the behaviour, the happy path plus failure and boundary cases, and deterministic fixtures — no sleeps, no network calls. Provide enough tests to satisfy the coverage gate below, and include the coverage tool + threshold in the build config so a run under the target fails.
- Lint-clean and consistently formatted: include the target stack's linter + formatter config (e.g. ESLint + Prettier, Ruff/Black, Checkstyle/Spotless) and produce code that passes it with zero errors — no TODOs, commented-out code, dead parameters or unused imports.
