...
- Tests accompany the code: arrange-act-assert, meaningful names stating the behaviour, the happy path plus failure and boundary cases, and deterministic fixtures — no sleeps, no network calls.
- Idiomatic for the target stack, documented at the public API level, and free of TODOs, commented-out code, dead parameters and unused imports.
